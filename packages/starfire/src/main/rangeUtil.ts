...
import {getSortedChildNodes} from './comments';

export const findSiblingAncestors = (startNodeAndParents, endNodeAndParents, opts) => {
  let resultStartNode: SFNode = startNodeAndParents.node;
  let resultEndNode: SFNode = endNodeAndParents.node;
...
export const isSourceElement = (opts, node: SFNode): boolean => {
  if(node === null) {
    return false;
  }
...
export const calculateRange = (text, opts, ast) => {
