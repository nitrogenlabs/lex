...
import {hasNewline, isPreviousLineEmpty, skipNewline} from '../common/util';
import {SFNode} from '../common/util.types';
import {addDanglingComment, addLeadingComment, addTrailingComment} from '../common/utilShared';
import {SFDocument, SFPart} from '../doc/doc.types';
import {breakParent, concat, cursor, hardline, indent, join, lineSuffix} from '../doc/docBuilders';

const childNodesCacheKey = Symbol('child-nodes');

export const getQuasiRange = (expr) => {
...
export const findExpressionIndexForComment = (quasis, comment, options) => {
...
export const getSortedChildNodes = (node, options, resultArray = []) => {
  if(!node) {
    return null;
  }
...
export const decorateComment = (node, comment, options) => {
...
export const breakTies = (tiesToBreak, text, options) => {
...
export const attach = (comments, ast, text, options) => {
...
export const printComment = (commentPath, options) => {
  const comment = commentPath.getValue();
  comment.printed = true;
  return options.printer.printComment(commentPath, options);
};

export const printLeadingComment = (commentPath, print, options) => {
  const comment = commentPath.getValue();
...
export const printTrailingComment = (commentPath: FastPath, print, options): SFPart => {
  const comment = commentPath.getValue();
  const contents = printComment(commentPath, options);
  if(!contents) {
    return '';
  }
  const isBlock = options.printer.isBlockComment && options.printer.isBlockComment(comment);

  // We don't want the line to break
  // when the parentParentNode is a ClassDeclaration/-Expression
  // And the parentNode is in the superClass property
  const parentNode: SFNode = commentPath.getNode(1);
  const parentParentNode: SFNode = commentPath.getNode(2);
  ...
  if(hasNewline(options.originalText, options.locStart(comment), {backwards: true})) {
...
export const printDanglingComments = (path: FastPath, options, sameIndent, filter) => {
  const parts = [];
...
export const prependCursorPlaceholder = (path: FastPath, options, printed) => {
...
export const printComments = (path: FastPath, print, options, needsSemi: boolean = false) => {
  ...
  const leadingParts: SFDocument[] = [];
  const trailingParts: SFDocument[] = [needsSemi ? ';' : '', printed];

  path.each((commentPath: FastPath) => {
    const comment = commentPath.getValue();
    ...
    if(leading) {
      ...
      if(index !== null && hasNewline(text, index)) {
