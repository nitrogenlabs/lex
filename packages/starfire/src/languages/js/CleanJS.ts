export class CleanJS {
  static clean(ast, newObj, parent) {
    ...
    if(
      ...
      const templateLiterals = newObj.children
        .filter((child) => child.type === 'JSXExpressionContainer' && child.expression.type === 'TemplateLiteral')
        .map((container) => container.expression);
      const quasis = templateLiterals.reduce((item, templateLiteral) => item.concat(templateLiteral.quasis), []);
