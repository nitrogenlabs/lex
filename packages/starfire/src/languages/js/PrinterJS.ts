...
export class PrinterJS {
  ...
  static printPathNoParens(path, options, print, args) {
    ...
    switch(n.type) {
      ...
      case 'Import':
        return 'import';
      case 'BlockStatement': {
        const naked = path.call((bodyPath) => PrinterJS.printStatementSequence(bodyPath, options, print), 'body');
      ...
      case 'SwitchCase': {
        ...
        if(consequent.length > 0) {
          const cons = path.call((consequentPath) => PrinterJS.printStatementSequence(consequentPath, options, print), 'consequent');
        ...
        return concat([
          ...
          n.body.length > 0
            ? indent(
              concat([
                hardline,
                path.call((bodyPath) => PrinterJS.printStatementSequence(bodyPath, options, print), 'body')
              ])
            )
            : Comments.printDanglingComments(path, options),
          hardline,
      ...
      case 'TSModuleBlock':
        return path.call((bodyPath) => PrinterJS.printStatementSequence(bodyPath, options, print), 'body');
  ...
  static printMemberChain(path, options, print) {
    ...
    const isFactory = (name) => name.match(/(^[A-Z])|^[_$]+$/);
    ...
    const printGroup = (printedGroup) => concat(printedGroup.map((tuple) => tuple.printed));
  ...
  static printJsDocComment(comment) {
    ...
    return concat([
      ...
      join(
        hardline,
        lines.map((lineItem, index) => (index > 0 ? ' ' : '') + (index < lines.length - 1 ? lineItem.trim() : lineItem.trimLeft()))
      ),
      '*/'
    ]);
  }
