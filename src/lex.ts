...
program.command('build')
  ...
  .option('--analyze', 'Generate a Vite bundle report and AI optimization suggestions.', false)
  .addOption(new Option('--bundler <name>', 'Bundler to use. Defaults to Vite for web and SWC otherwise.').choices(['vite', 'swc']))
  .option('--entry <value...>', 'Entry module. When multiple values are supplied, Lex uses the first.')
  ...
  .addOption(new Option('--mode <type>', 'Build mode. Default: "production".').choices(['development', 'production']).default('production'))
  .option('--outputPath <value>', 'Output directory. Relative paths resolve from the project directory.')
...
program.command('storybook')
  ...
  .option('--port <number>', 'Port number for the Storybook server.', '6007')
  .option('--quiet', 'No Lex notifications printed in the console.')
  .option('--static', 'Build a static Storybook site instead of starting dev server.')
  .option('--useLexConfig', 'Use Lex\'s React Vite Storybook configuration instead of the project configuration.')
  .option('--variables <n>', 'Environment variables to set in "process.env". (ie. "{STORYBOOK_THEME: \'dark\'}").')
  .option('--verbose', 'Show detailed Storybook build progress output.')
...
program.command('test [files...]')
  ...
  .option('--changedSince <branch>', 'Run tests related to changes since the provided branch.')
  ...
  .option('--environment <n>', 'Target environment. "node" or "web". Default: "node".')
  .option('--env <name>', 'Vitest environment, such as happy-dom, jsdom, or node.')
  ...
  .option('--onlyChanged', 'Run tests related to files changed in the current repository.')
...
program.command('update')
  ...
  .addOption(new Option('--package-manager <manager>', 'Which package manager to use. Default: npm').choices(['npm', 'yarn']).default('npm'))
  .option('--quiet', 'No Lex notifications printed in the console.')
  .option('--registry <url>', 'Registry used while updating packages.')
